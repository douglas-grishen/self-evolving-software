...
async def test_runtime_contract_probe_422_is_excluded_from_error_metrics():
    ...
    assert snapshot["total_client_errors"] == 0
    assert metrics_module.get_recent_errors() == []


@pytest.mark.asyncio
async def test_normal_422_counts_as_client_error_not_runtime_error():
    """Regular client validation failures should remain visible without paging the engine."""
    _reset_metrics_state()
    middleware = MetricsMiddleware(app=lambda scope, receive, send: None)
    ...
    assert snapshot["global_error_rate"] == 0.0
    assert metrics_module.get_recent_errors() == []
